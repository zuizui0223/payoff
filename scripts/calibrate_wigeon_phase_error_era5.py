#!/usr/bin/env python3
"""Calibrate wigeon phase-reconstruction sensitivity with ERA5.

The scientific contract is frozen in
data/wigeon_phase_error_era5_sourcefaithful_registration_20260924.json.

This script retrieves Open-Meteo ERA5 hourly 2 m temperature at the original
reconstructed staging-event coordinates, computes arithmetic GMT daily means
from exactly 24 finite hourly values, applies the source-faithful January-July
5 C cumulative-minimum TGS rule, pairs the result with the frozen NASA POWER
reconstruction, refits the exact wigeon controller, and runs the frozen
true-lambda=1 sensitivity nulls.
"""
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=Path,
        default=Path(
            "outputs/wigeon_power_era5_event_calibration.csv"
        ),
    )
    p.add_argument(
        "--transitions-output",
        type=Path,
        default=Path(
            "outputs/wigeon_power_era5_transition_calibration.csv"
        ),
    )
    p.add_argument(
        "--raw-request-output",
        type=Path,
        default=Path(
            "outputs/wigeon_era5_request_receipt.json"
        ),
    )
    p.add_argument(
        "--receipt-output",
        type=Path,
        default=Path(
            "outputs/wigeon_phase_error_era5_calibration_receipt.json"
        ),
    )
...
def request_batch(session, endpoint, rows, year, *, max_retries):
    ...
    raise RuntimeError(
        f"Open-Meteo ERA5 request failed after {max_retries} attempts: {last}"
    )


def finite_daily_pairs(payload):
    """Aggregate ERA5 hourly 2 m temperature to strict GMT daily means."""
    ...
    for day in sorted(by_day):
        ...
        if len(values) != 24:
            continue
        daily.append((day, sum(values) / 24.0))
    return daily
...
def extract_era5_events(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "wigeon ERA5 calibration requires pandas, numpy and requests"
        ) from exc
                ...
                rows.append(
                    {
                        ...
                        "era5_tgs_onset_doy": era5_tgs,
                        "era5_phase_days": era5_phase,
                        "era5_minus_power_phase_days": phase_difference,
    ...
    if result.duplicated(["individual_id", "year", "segment"]).any():
        raise ValueError("duplicate staging-event identity after ERA5 query")
        ...
        rows.append(
            {
                **row._asdict(),
                ...
                "era5_origin_phase": (
                    np.nan
                    if origin.era5_phase_days is None
                    else float(origin.era5_phase_days)
                ),
                "era5_destination_phase": (
                    np.nan
                    if destination.era5_phase_days is None
                    else float(destination.era5_phase_days)
                ),
                "origin_replicate_difference": (
                    np.nan
                    if origin.era5_minus_power_phase_days is None
                    else float(origin.era5_minus_power_phase_days)
                ),
                "destination_replicate_difference": (
                    np.nan
                    if destination.era5_minus_power_phase_days is None
                    else float(destination.era5_minus_power_phase_days)
                ),
            }
        )
...
def phase_validation(events, registration):
    ...
    phase = valid["era5_phase_days"].astype(float)
...
def main():
    ...
    try:
        ...
        raise RuntimeError(
            "wigeon ERA5 calibration requires empirical dependencies"
        ) from exc
    ...
    coverage_gate = (
        ...
        and not (
            ...
            and (
                valid_events["era5_tgs_onset_doy"].astype(float) >= 300
            ).any()
        )
    )
    ...
    disagreement = summarize_replicate_disagreement(
        valid_events["era5_minus_power_phase_days"].astype(float)
    )
    ...
    receipt = {
        ...
        "era5_controller": asdict(era5_fit),
        "controller_difference": {
            "paired_era5_minus_power_lambda_hat": (
                era5_fit.lambda_hat - power_fit_paired.lambda_hat
            ),
            "paired_era5_minus_power_stopover_slope": (
        ...
        "claim_boundary": [
            "POWER-ERA5 disagreement is a replicate-disagreement calibration, not a gold-standard error estimate.",
    ...
    if (
        ...
        raise SystemExit("wigeon ERA5 calibration gate failed")
