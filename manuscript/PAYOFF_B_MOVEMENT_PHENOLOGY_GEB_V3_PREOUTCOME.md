...
**Methods:** We tested a universal movement-to-phenology speed ratio in 5,816 bird-year-cell observations from 55 species, then estimated signed phase retention from E_next = a + λE_current + ε over declared movement intervals. Before opening the Aikens phase-retention outcome, we froze secondary interval-standardized summaries using an equivalent decay constant k_eq = −ln|λ|/Δt_ref and cumulative retained-memory R_path = |λ|^n. We reconstructed three direct taxa, estimated environmental timing innovation separately, and tested an industrial-development perturbation.

**Results:** Broad bird data rejected one universal natural speed optimum. Raw phase-retention estimates were about 0.11 in mule deer, 0.11–0.49 across highlighted barnacle-goose transitions, and 0.75 in wigeon, but median interval lengths differed from 1.06 d in wigeon to 47 d for whole-migration mule deer. Using the frozen secondary standardization, the typical seven-transition wigeon retained-memory component was 0.133 under POWER and 0.231 under ERA5, the same order as whole-migration mule-deer retention (0.107), although the conservative wigeon SIMEX sensitivity allowed 0.627. Environmental predictability did not map monotonically onto correction strength. <!-- AIKENS_LAMBDA_ABSTRACT_START -->
[AIKENS LAMBDA ABSTRACT PENDING — render from the registered result JSON before submission.]
<!-- AIKENS_LAMBDA_ABSTRACT_END -->

**Main conclusions:** Phase retention is useful as a common estimator form, but raw λ magnitudes are interval-scale dependent and should not be ranked as portable biological constants. Interval-standardized cumulative retained memory provides a more interpretable cross-system secondary comparison, while measurement-error uncertainty prevents a universal migration-wide correction fraction. Environmental information, realized correction and physical actuators remain separable channels.
