...
12. anisotropic and directionally biased movement kernels plus algebraic
    observation-to-parameter maps — implemented;
13. projected fixed-interval calibration and phase-controller refusal audits —
    implemented;
14. mule-deer named-system readiness / refusal audit — implemented;
15. named-system interval calibration — pending source-file ingestion.
...
identifiable subset of tracking parameters under the declared movement family.
For mean-zero projected interval steps, component second moments identify
migration rate and x/y movement weights. For directional migration, the same
second moments together with component means identify x/y directional biases.
Environmental wave speed times spatial gradient maps to climate-coordinate
velocity.

Phase-error compression remains a separate controller estimand. It identifies
the independent phenology response rate only when timing has been isolated from
spatial movement and other correction pathways. A separate matched-contrast
design identifies baseline growth, abiotic and interaction mismatch strengths,
and quadratic tracking costs. These maps are model-conditional and do not yet
...
The interval pipeline can now parameterize a directionally biased movement
kernel from projected fixed-interval GPS means and second moments, but the
published summaries do not contain those quantities.

Group-level early and mid Days-From-Peak means cross the zero boundary, and the
monotone late-group compression is measured across a whole migration rather
than a fixed per-step interval. Because phase compression is itself partly
generated by movement speed and stopover adjustment, those group summaries are
also not treated as the independent timing-axis h. The repository therefore
